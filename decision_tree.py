...
def entropy(y):
    ...
    N = len(y)
    s1 = (y == 1).sum() #find all the data for which y==1
    if 0 == s1 or N == s1:
        return 0 #0 entropy
    p1 = float(s1) / N
    p0 = 1 - p1
...
class TreeNode:
    # every node should know its own depth and Tree's max_depth
    ...
    def fit(self, X, Y):
        if len(Y) == 1 or len(set(Y)) == 1: # base case
            ...
            D = X.shape[1] # No of columns in our data
            cols = range(D) # column index

            max_ig = 0
            best_col = None
            best_split = None
            # looping through X columns trying to find the best split
            for col in cols:
    ...
    def find_split(self, X, Y, col):
        ...
        # this tells you when label changes from another label to another label
